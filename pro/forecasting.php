  ...
  <aside class="main-sidebar">
    ...
    <li class="treeview">
          ...
          <ul class="treeview-menu">
            <li><a href="bus.php"><i class="fa fa-circle-o"></i> Data Bus</a></li>
    ...
    <section class="content">
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title text-primary"><i class="fa fa-calculator"> </i> Forecasting</h3>
        ...
        <!-- form start -->
			  ...
			  <div class="box-body">
          <a href="#" onclick="window.print()" class="pull-right text-primary">Cetak</a><br/>
					<?php 
            $result = $mysqli->query("select * from `simulasi-bulan`");
              $jml = $result->num_rows; //jumlah data
              $data = array();
              $i = 1;
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                $data["id"][$i] = $row["id"]; 
                $data["bulan"][$i] =$row["bulan"];    
                $data["jumlah"][$i] =$row["jumlah"];
              $i++;
            }

            $ram = "bulan"; //yg akan diramal
            $num = 0.1; //nilai alpha
            ...
            // Input User
            echo "<b>Data User</b> <table>";
            echo "<tr><td>Jumlah Data</td><td>: ".$jml."</td></tr>";
            echo "<tr><td>Yang akan diramal</td><td>: ".ucwords($ram)." berikutnya</td></tr>";
            ...
            for($i=1;$i<=$jml;$i++){
              echo "<tr><td>Ft".$i."</td><td>".$data["bulan"][$i]."</td><td>".number_format($data["jumlah"][$i], 0, '', '.')."</td></tr>";
            }
            echo "</table></div>";
            ...
            for($i=1;$i<=$jml+1;$i++){
              if($i>=4){ 
                ...
                if($i==$jml+1) echo "<tr><td>Ft".$i."</td><td></td><td>".number_format(rou($temp), 2, ',', '.')."</td><td>Rata-rata : ".number_format(rou($maema/($jml-3)), 2, ',', '.')."</td><td>Rata-rata : ".number_format(rou($msema/($jml-3)), 2, ',', '.')."</td><td>Rata-rata : ".round(($mapema/($jml-3))*100)."%</td></tr>";
                else {
                  $temp2 = abs($da[$i]-$temp);
                  $temp3 = pow($temp2,2);
                  $temp4 = abs($temp2)/$da[$i];
                  echo "<tr><td>Ft".$i."</td><td>".number_format($da[$i], 0, '', '.')."</td><td>".number_format(rou($temp), 2, ',', '.')."</td><td>".number_format(rou($temp2), 2, ',', '.')."</td><td>".number_format(rou($temp3), 2, ',', '.')."</td><td>".round($temp4*100)."%</td></tr>";
                }
                $ma[$i] = $temp;
                $maema = $maema + $temp2;
                $msema = $msema + $temp3;
                ...
                echo "<tr><td>Ft".$i."</td><td>".number_format($da[$i], 0, '', '.')."</td><td></td><td></td><td></td><td></td></tr>";
            }
            echo "</table></div>";
            ...
            for($i=1;$i<=$jml+1;$i++){
              if($i>=2){ 
                ...
                if($i==$jml+1) echo "<tr><td>Ft".$i."</td><td></td><td>".number_format(rou($temp), 2, ',', '.')."</td><td>Rata-rata : ".number_format(rou($maees/($jml-1)), 2, ',', '.')."</td><td>Rata-rata : ".number_format(rou($msees/($jml-1)), 2, ',', '.')."</td><td>Rata-rata : ".round(($mapees/($jml-1))*100)."%</td></tr>";
                else{ 
                  $temp2 = abs($da[$i]-$temp);
                  $temp3 = pow($temp2,2);
                  $temp4 = abs($temp2)/$da[$i];
            ...
            echo "<br>Maka perkiraan jumlah penumpang pada ".$ram." berikutnya adalah <b>".number_format(round($ma[$jml+1]), 0, '', '.')." dengan metode Moving Average</b>, dan <b>".number_format(round($es[$jml+1]), 0, '', '.')." dengan metode Exponential Smoothing</b>.";
